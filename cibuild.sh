...
function arm-gcc-toolchain {
  ...
  if [ ! -f "${tools}/gcc-arm-none-eabi/bin/arm-none-eabi-gcc" ]; then
    ...
    case ${os} in
      Darwin)
        archivertool=tar
        namefile=arm-gnu-toolchain-13.2.rel1-darwin-x86_64-arm-none-eabi
        ;;
      Linux)
       archivertool=tar
        namefile=arm-gnu-toolchain-13.2.rel1-x86_64-arm-none-eabi
        ;;
      MSYS*)
        archivertool=unzip
        namefile=arm-gnu-toolchain-13.2.rel1-mingw-w64-i686-arm-none-eabi
        ;;
    esac
    cd "${tools}"
    if [ "$archivertool" == "tar" ]; then
      wget --quiet https://developer.arm.com/-/media/Files/downloads/gnu/13.2.rel1/binrel/${namefile}.tar.xz
      xz -d ${namefile}.tar.xz
      tar xf ${namefile}.tar
      mv ${namefile} gcc-arm-none-eabi
      rm ${namefile}.tar
    else
      wget --quiet https://developer.arm.com/-/media/Files/downloads/gnu/13.2.rel1/binrel/${namefile}.zip
      unzip -qo ${namefile}.zip
      mv ${namefile} gcc-arm-none-eabi
      rm ${namefile}.zip
    fi
  fi
...
case ${os} in
  ...
  Linux)
    ...
    ;;
  MSYS*)
    install="arm-gcc-toolchain kconfig-frontends c-cache"
    ;;
esac
